...
El corte que acabé haciendo es de una sola frase: `install.sh` decide **qué** se hace y **en qué orden**; `lib/` sabe **cómo** se hace. Una fase por archivo. El instalador dejó de ser un procedimiento y pasó a ser un índice de contenidos.
...
Así que `--agent` instala lo que ese usuario sí usa: herramientas de línea de comandos, hooks de git y `~/.claude` (settings, statusline, CLAUDE.md). Y no instala nada interactivo. No es una versión recortada por ahorrar, es la lista correcta para ese destino.

<Callout type="note">
Tu entorno de desarrollo ya tiene dos usuarios. Solo uno de los dos ve los colores.
</Callout>

El giro está en qué queda cuando quitas la decoración. Lo que sobrevive son las reglas: los hooks que rechazan un mensaje de commit inservible, los que impiden un commit directo a `main`, los que comprueban que la documentación sigue el paso del código. Un agente que trabaja en tu repositorio necesita precisamente eso, y por las mismas razones por las que [la disciplina de desarrollo con Claude Code](/es/blog/agentic-saas-b2b-disciplina-de-desarrollo-con-claude-code) importa más que el prompt: las restricciones que se ejecutan valen más que las instrucciones que se leen. De eso trata la tercera parte de esta serie.
